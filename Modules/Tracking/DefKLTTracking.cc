...
namespace defSLAM
{
                 ...
                 strSettingPath, sensor, viewerOn)
  {
    ...
    templatefile.close();
    ofstream computationTime("computationTime.txt");
    computationTime.close();
    ofstream BBStime("mapbbs.txt", std::ios::in | std::ios::out | std::ios::ate);
    BBStime.close();
    ofstream maptime("maptime.txt", std::ios::in | std::ios::out | std::ios::ate);
    maptime.close();
    ///-------------------------------
  ...
  void DefKLTTracking::Track()
  {
    ...
    else
    {
      ...
      if (!mbOnlyTracking)
      {
        ...
        if (mState == OK)
        {
          ofstream computationTime("computationTime.txt", std::ios::in | std::ios::out | std::ios::ate);
          ...
          auto start_map = std::chrono::steady_clock::now(); //模板重建开始时间
          auto end_map = std::chrono::steady_clock::now();   //模板重建结束时间
          if ((static_cast<DefLocalMapping *>(mpLocalMapper)
                   ->updateTemplate()))
          {
            end_map = std::chrono::steady_clock::now(); //模板重建结束时间
            mpReferenceKF =
          ...
          auto end_all = std::chrono::steady_clock::now(); //结束时间
          auto duration_whole = std::chrono::duration_cast<std::chrono::milliseconds>(end_all - start_all);
          auto duration_map = std::chrono::duration_cast<std::chrono::milliseconds>(end_map - start_map);
          computationTime<<mCurrentFrame->mTimeStamp<<" "<<duration_map.count()<<" "<<duration_whole.count()<<endl;
          computationTime.close();
        }
        else
        {
                                               ...
                                               cv::Mat _mask)
  {
    ...
    if ((mState == eTrackingState::OK) && (saveResults))
    {
      ...
      scalefile.open("ScaleVariation.txt", std::ios::in | std::ios::out | std::ios::ate);
      scalefile << mCurrentFrame->mTimeStamp << " " << scale << " " << mCurrentFrame->mpReferenceKF->mTimeStamp << std::endl;
      scalefile.close();
      double error = static_cast<GroundTruthFrame *>(mCurrentFrame)
                         ->Estimate3DError(mpMap, scale);
      RMSE.open("RMSE.txt", std::ios::in | std::ios::out | std::ios::ate);
      RMSE<<mCurrentFrame->mTimeStamp<<" "<<error*1000<<std::endl;
      RMSE.close();
