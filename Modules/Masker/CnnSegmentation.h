...
namespace defSLAM{
    ...
    public:
        ...
        void loadRindNetModel(std::string path);

        cv::Mat generateMask(const cv::Mat& im){
            return forward(im);
        }
        cv::Mat extractContour(const cv::Mat& im);
        void contourSamples(const cv::Mat& im, std::vector<std::vector<cv::Point>> &contours_sampled, float sample_rate, int contour_num, int sample_max_num);
    ...
    private:
        ...
        cv::Mat depthTorchToMat(at::Tensor &tensor);
        cv::Mat normalTorchToMat(at::Tensor &tensor);
        ...
        cv::Mat postprocessContour(const cv::Mat &depth_im, const cv::Mat &normal_im);
        cv::Mat skeletonProcess(cv::Mat &sourceBinaryImage);
        ...
        torch::jit::script::Module rindNetModel_;
    };
}
