...
namespace defSLAM
{
  ...
  */
  double GroundTruthFrame::Estimate3DScale(Map *map)
  {
    ...
    kps.reserve(vpMPs.size());
    std::vector<float> zs;
    bool libelas(false); // for DefSLAMGT, set libelas=true; for DefSLAMGTCT, set libelas=false
    if (libelas)
    {
    ...
    else
    {
      ...
      if (!isDepth)
      {
        ...
        errorKp_.reserve(posStereoInit_.size());
        ...
        for (uint i(0); i < notOutliers.size(); i++)
        {
          ...
          errorKp_.push_back(kps[notOutliers[i]]);
        }
        if (posMono_.size() < 20)
          return 1;
        filtering_time =
  ...
  */
  double GroundTruthFrame::Estimate3DError(Map *map, const double &s)
  {
    ...
    GroundTruthTools::saveResultsWithKp(errorKp_, Error, name);
    return acc * invc;
  }
