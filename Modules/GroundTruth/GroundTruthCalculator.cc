 ...
 * *******************/
    ...
    void saveResultsWithKp(std::vector<cv::KeyPoint> &vectorKp, std::vector<float> &vectorError, std::string &name){
      Eigen::MatrixXd asd(vectorError.size(), 3);
      for (uint i(0); i < vectorError.size(); i++)
      {
        asd(i, 0) = vectorKp[i].pt.x;
        asd(i, 1) = vectorKp[i].pt.y;
        asd(i, 2) = vectorError[i];
      }
      std::ofstream myfile;
      myfile.open(name);
      myfile << asd;
 ...
 * *******************/
                                                      ...
                                                      double cx, double cy, double fx, double fy)
    {
      ...
      // cv::imwrite("imGTlibelas.png", imRight);
      ...
      for (auto &kp : kp)
      {
        ...
        if ((libdisp < 200) and (libdisp > 40) and (kp.pt.x >= 0))
        {
          std::vector<float> ps;
          ps.reserve(3);
