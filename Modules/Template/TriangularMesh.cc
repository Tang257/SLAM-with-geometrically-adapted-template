...
namespace defSLAM
{
      ...
      : Template(mspMapPoints, map, kf)
  {
    ...
    std::vector<std::vector<int>> adaptivefacets;
    cv::Mat imRGB = static_cast<DefKeyFrame *>(kf)->KFimage.clone();
    // cv::imwrite("/home/tang/GithubRep/SD-DefSLAM/datasets/phantom_datasets/processInfo/reference_imRGB.png", imRGB);
    nodVer = 2;
    nodHor = 2;

    std::cout<<"nodVer: "<<nodVer<<" nodHor: "<<nodHor<<std::endl;
    RefSurface->getAdaptiveVertex(NodesSurface, adaptivefacets, nodVer, nodHor, imRGB.cols, imRGB.rows, static_cast<DefKeyFrame *>(kf)->_contoursSamples);
    auto facets = adaptivefacets;

    ofstream templatefile("template.txt", std::ios::in | std::ios::out | std::ios::ate);
    templatefile<<kf->mTimeStamp<<" "<<NodesSurface.size()<<" "<<facets.size()<<endl;
    templatefile.close();

    // // int nodVer = 15;
    // // int nodHor = 15;
    // // 使用Bibubic B-Spline插值法，得到nodVer*nodHor个点
    // // std::vector<cv::Mat> NodesSurface;
    // RefSurface->getVertex(NodesSurface, nodVer, nodHor); // 从RefSurface得到nodVer行nodHor列顶点的NodesSurface
    // // 从点中计算通用的三角面
  ...
  void TriangularMesh::calculateFeaturesCoordinates()
  {
    std::for_each(
        ...
        std::cout<<"facets in template: "<<facets_.size()<<std::endl;
        std::cout<<"nodes in template: "<<nodes_.size()<<std::endl;
  }
