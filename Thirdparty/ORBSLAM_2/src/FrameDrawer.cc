...
namespace ORB_SLAM2
{
  ...
  FrameDrawer::FrameDrawer(Map *pMap) : error(0.0), mpMap(pMap)
  {
  ...
  cv::Mat FrameDrawer::DrawFrame()
  {
    ...
    vector<cv::KeyPoint> vMapKeys;     // Potential Map Points in current frame
    ...
    {
      for (auto kp : mvCurrentLocalMap)
      {
        cv::circle(im, kp.pt, 2, cv::Scalar(240, 255, 255), -1);
      }
      for (auto kp : vMapKeys)
      {
        cv::circle(im, kp.pt, 2, cv::Scalar(255, 255, 255), -1);
      }
      mnTracked = 0;
      mnTrackedVO = 0;
      const float r = 5;
  ...
  cv::Mat FrameDrawer::getImageTexture(){
    cv::Mat im;
    {
      unique_lock<mutex> lock(mMutex);
      mIm.copyTo(im);
  ...
  void FrameDrawer::SetError(double s)
  {
    std::unique_lock<std::mutex> mut(this->MutexText);
    this->error = s;
  }
  ...
  void FrameDrawer::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText)
  {
    ...
    else if (nState == Tracking::OK)
    {
      ...
      s << " |  RMSE : " << int(1000 * this->error) << " mm " << std::endl;
    }
  ...
  void FrameDrawer::Update(Tracking *pTracker)
  {
    ...
    this->mvbMapcorr = vector<bool>(N, false);
